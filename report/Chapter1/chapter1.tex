...
\begin{itemize}
\item **Xây dựng quy trình chẩn đoán sự cố trên nền tảng NIKA:** Nghiên cứu và ứng dụng môi trường giả lập mạng Kathará để triển khai các kịch bản sự cố phức tạp (như BGP ASN mismatch, link down, host crash, flow rule loop). Đồng thời thiết lập các AI Agent chẩn đoán hoạt động dựa trên các luồng suy luận phổ biến như ReAct, Plan \& Execute, và Reflexion.

\item **Đề xuất và hiện thực hóa Mô-đun Bộ Nhớ Quy Trình Tiến Hóa (Composable Procedural Memory):** Thiết kế bộ lưu trữ dữ liệu lai giữa PostgreSQL (quan hệ nguyên tử) và Qdrant (vector index). Ứng dụng các cơ chế truy xuất thuộc tính ngữ cảnh (MemInsight), nén vết chẩn đoán (LightMem) và liên kết đồ thị (A-Mem) để giúp agent tích lũy kinh nghiệm chẩn đoán hiệu quả. Đồng thời thiết kế cơ chế score-gated validation dựa trên điểm Evaluator của NIKA nhằm bảo vệ bộ nhớ khỏi hiện tượng học vẹt đáp án benchmark.

\item **Đề xuất và hiện thực hóa Mô-đun Tiến Hóa Công Cụ (Tool Evolution):** Thiết kế cơ chế cho phép AI Agent tự hoàn thiện tài liệu hướng dẫn công cụ (Mastery) và sinh các công cụ chẩn đoán mới ở test-time (composite workflows và generated Python helpers). Xây dựng cơ chế xác thực an toàn mã nguồn sinh ra bằng cách phân tích AST và thực thi trong môi trường Docker sandbox cô lập.

\item **Đánh giá thực nghiệm chi tiết và sâu sắc:** Thực hiện kiểm thử agent tự tiến hóa trên 30 sự cố chọn lọc từ benchmark NIKA, so sánh toàn diện với baseline về các chỉ số: độ chính xác chẩn đoán (detection, localization, RCA), số bước chẩn đoán, số lượng công cụ và lượng token tiêu thụ.

\item **Phân tích thách thức trong thực tế:** Chỉ ra các thách thức còn tồn tại khi áp dụng Self-Evolving AI Agents vào môi trường mạng viễn thông thực tế, chẳng hạn như chất lượng dữ liệu vận hành, độ tin cậy của phản hồi, rủi ro học sai từ kinh nghiệm quá khứ, khả năng kiểm soát quá trình tự cập nhật và yêu cầu đảm bảo an toàn trong các hệ thống quan trọng.
