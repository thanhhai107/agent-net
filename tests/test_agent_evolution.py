...
class AgentEvolutionTest(unittest.TestCase):
    ...
    def test_run_harness_evolution_creates_target_agent_for_next_generation(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with (
                ...
                patch(
                    "nika.workflows.evolve.run._invoke_target_source_agent",
                    return_value=TargetAgentArtifact(
                        improvement_md="# Improvement\n\n- generated by source agent\n",
            ...
            self.assertIn("generated by source agent", improvement.read_text())
    ...
    def test_meta_and_feedback_prompts_keep_benchmark_tools_fixed(self) -> None:
        ...
        feedback_prompt = _build_feedback_prompt(
            ...
            current_source=_reference_target_agent_source(),
        )

        for prompt in (initial_prompt, feedback_prompt):
            self.assertIn("MCP tools", prompt)
            self.assertIn("Do not invent new benchmark primitive tools", prompt)
            self.assertIn("tool surface", prompt)

    def test_public_case_dataset_declares_fixed_tool_contract(self) -> None:
        class FakeSession:
            session_id = "session-1"
            scenario_name = "ospf_enterprise_static"
            scenario_topo_size = ""
            topology = []
            task_description = "Diagnose the public network incident."
            session_dir = ""

        with tempfile.TemporaryDirectory() as tmp:
            dataset_dir = build_public_case_dataset(
                session=FakeSession(),
                output_dir=Path(tmp) / "public_case",
                memory=MemoryConfig(),
                tool_evolution=ToolEvolutionConfig(),
                llm_backend="openai",
                model="test-model",
            )
            context = json.loads((dataset_dir / "case_context.json").read_text())

        self.assertEqual(context["tool_contract"]["surface"], "fixed_mcp_tools")
        self.assertFalse(context["tool_contract"]["may_create_new_primitive_tools"])
        self.assertFalse(context["tool_contract"]["may_create_new_mcp_servers"])

    def test_target_agent_validation_rejects_embedded_tool_servers(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "target_agent.py"
            path.write_text(
    ...
    def test_feedback_context_uses_metrics_and_execution_not_hidden_run_labels(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            session_dir = root / "20260629-120332-1a7287"
            session_dir.mkdir(parents=True)
            (session_dir / "run.json").write_text(
                json.dumps(
                    {
                        ...
                        "scenario_name": "ospf_enterprise_static",
                        "problem_names": ["hidden_problem_label"],
                        "benchmark_index": 0,
                    }
                ),
                encoding="utf-8",
            ...
            (session_dir / "agent_execution.json").write_text(
                json.dumps(
                    {
                        "diagnosis_report": "execution trajectory is visible",
                        "submission_result": "target output is visible",
                        "messages": [
                            {
                                "event": "tool_end",
                                "content": "trajectory observation text",
                            }
                        ],
                    }
                ),
        ...
        self.assertNotIn("hidden_problem_label", context)
        self.assertIn("Case Metrics", context)
        self.assertIn("Execution Samples", context)
        self.assertIn("execution trajectory is visible", context)
        self.assertIn("target output is visible", context)

    def test_feedback_digest_omits_runtime_extension_context(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(tmp) / "agent_execution.json"
            path.write_text(
                json.dumps(
                    {
                        "diagnosis_report": "used available context",
                        ...
                        "messages": [
                            {
                                ...
                                "content": (
                                    ...
                                    "Never inspect host X before command Y.\n"
                                    "\n# DRAFT tool documentation note\n"
                                    "Use exact device names for tool Z.\n"
                                ),
                            }
                        ],
                    }
            ...
            digest = _execution_digest(path)

        assert digest is not None
        content = digest["messages_tail"][0]["content"]
        self.assertIn("<runtime-extension-context omitted>", content)
        self.assertNotIn("Never inspect host X", content)
        self.assertNotIn("Prefer promoted tool Z", content)
    ...
    def test_llm_feedback_agent_output_can_drive_next_target(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(source, "feedback-agent")
            self.assertIn("tighten RCA", improvement)
            self.assertTrue(next_target.is_file())
            feedback_agent.assert_called_once()

    def test_feedback_agent_error_is_not_silently_carried_forward(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            tmp_path = Path(tmp)
            current = tmp_path / "current.py"
            current.write_text(_reference_target_agent_source(), encoding="utf-8")
            ...
            self.assertFalse(next_target.exists())
