"""Integration tests for PingMesh MCP snapshot probing."""
...
from tests.integration.pipeline_case import tool_text_list
from tests.integration_base import PerTestEnvTestCase
from tests.integration_pipeline import _min3clos_prerequisites
from tests.net_env_verify.helpers import docker_available


def _invoke_pingmesh(session_id: str) -> dict:
    config = MCPServerConfig(session_id=session_id).load_filtered_config(
        ["pingmesh_mcp_server"]
    )
...
class KatharaPingMeshIntegrationTest(PerTestEnvTestCase):
    ...
    def test_pingmesh_healthy_then_faulty(self) -> None:
        healthy = _invoke_pingmesh(self.session_id)
        ...
        faulty = _invoke_pingmesh(self.session_id)
