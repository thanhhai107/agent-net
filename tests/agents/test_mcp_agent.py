...
from tests.integration_pipeline import CommonPipelineSteps, load_test_env, openai_api_key_available
...
class McpAgentConfigTest(unittest.TestCase):
    ...
    def test_model_from_env(self) -> None:
        with unittest.mock.patch.dict(os.environ, {ENV_MCP_AGENT_MODEL: "gpt-4.1-mini"}, clear=True):
            self.assertEqual(resolve_agent_model("byo.mcp_agent", None), "gpt-4.1-mini")
            self.assertEqual(resolve_agent_model("byo.mcp_agent", "gpt-4o-mini"), "gpt-4o-mini")
...
@unittest.skipUnless(openai_api_key_available(), "OPENAI_API_KEY required for byo.mcp_agent")
