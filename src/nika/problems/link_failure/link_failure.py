...
class LinkFrag(ProblemBase):
    ...
    def inject_fault(self, params: LinkFragParams):
        ...
        match self.lab_backend:
            ...
            case "containerlab":
                self._inject_link_frag_containerlab(params)
            case backend:
                raise RuntimeCapabilityError(
                    f"{type(self).__name__} cannot inject_fault: unsupported backend {backend!r}."
                )

    def _inject_link_frag_kathara(self, params: LinkFragParams) -> None:
        self._inject_link_frag(params)

    def _inject_link_frag_containerlab(self, params: LinkFragParams) -> None:
        self._inject_link_frag(params)

    def _inject_link_frag(self, params: LinkFragParams) -> None:
        self.mtu = params.mtu
        self.runtime.exec(
            params.host_name,
    ...
    def verify_fault(self, params: LinkFragParams) -> dict:
        ...
        match self.lab_backend:
            case "kathara":
                return self._verify_link_frag_kathara(params)
            case "containerlab":
                return self._verify_link_frag_containerlab(params)
            case backend:
                raise RuntimeCapabilityError(
                    f"{type(self).__name__} cannot verify_fault: unsupported backend {backend!r}."
                )

    def _verify_link_frag_kathara(self, params: LinkFragParams) -> dict:
        return self._verify_link_frag(params)

    def _verify_link_frag_containerlab(self, params: LinkFragParams) -> dict:
        return self._verify_link_frag(params)

    def _verify_link_frag(self, params: LinkFragParams) -> dict:
