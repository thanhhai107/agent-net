...
class OSPFEnterpriseDHCP(NetworkEnvBase):
    ...
    def __init__(self, topo_size: Literal["s", "m", "l"] = "s"):
        ...
        for core_id, core_meta in core_routers.items():
            ...
            core_meta.machine.create_file_from_path(
                str(pkg_path("net_env/kathara/utils/ospf/vtysh.conf")), "/etc/frr/vtysh.conf"
            )
        ...
        for web_idx, web_meta in enumerate(web_servers):
            web_meta.machine.create_file_from_path(
                str(pkg_path("net_env/kathara/utils/web/web_server.py")), "web_server.py"
            )
            web_meta.machine.create_file_from_path(
    ...
    def verify_lab(self) -> dict:
        from nika.net_env.kathara.intradomain_routing.ospf_enterprise.verify import verify_ospf_enterprise_lab
