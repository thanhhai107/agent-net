...
def parse_clab_topology(path: Path | str) -> LabSpec:
    ...
    if not isinstance(data, dict):
        raise ValueError(f"Invalid containerlab topology at {path}: expected mapping at root.")

    lab_name = str(data.get("name") or "")

    topology = data.get("topology")
    if not isinstance(topology, dict):
        raise ValueError(f"Invalid containerlab topology at {path}: missing topology section.")

    raw_nodes = topology.get("nodes") or {}
    if not isinstance(raw_nodes, dict):
        raise ValueError(f"Invalid containerlab topology at {path}: topology.nodes must be a mapping.")
    ...
    if not isinstance(raw_links, list):
        raise ValueError(f"Invalid containerlab topology at {path}: topology.links must be a list.")

    links: list[LinkSpec] = []
    for idx, link_cfg in enumerate(raw_links):
        if not isinstance(link_cfg, dict):
            raise ValueError(f"Invalid link at index {idx} in {path}: expected mapping.")
