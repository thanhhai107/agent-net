...
* **diagnosis phase** → :class:`~agent.codex_cli.phases.CodexCliDiagnosisPhase`
  (``codex exec`` with Kathara MCP servers; server set chosen dynamically
  based on the session scenario)
* **submission phase** → :class:`~agent.codex_cli.phases.CodexCliSubmissionPhase`
...
Select with ``nika agent run -a codex_cli``.
"""
